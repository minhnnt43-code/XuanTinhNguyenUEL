...
document.addEventListener('DOMContentLoaded', () => {
    ...
    onAuthStateChanged(auth, async (user) => {
        ...
        // ============================================================
        const ALLOWED_ROLES = ['super_admin', 'kysutet_admin', 'doihinh_admin'];
...
function setupMenuByRole() {
    ...
    if (role === 'member') {
        ...
        // Thêm quyền xem Danh sách Chiến sĩ cho đội trưởng/đội phó
        document.getElementById('menu-members')?.classList.remove('hidden');
...
async function loadMembers() {
    ...
    try {
        ...
        // ========== FILTER: CHỈ HIỂN THỊ BCH CÁC CẤP ==========
        // Loại bỏ Chiến sĩ (role='member' hoặc position='Chiến sĩ')
        const originalCount = membersDataCache.length;
        membersDataCache = membersDataCache.filter(m => {
            // Giữ lại nếu role KHÔNG phải member
            if (m.role && m.role !== 'member') return true;
            // Hoặc position KHÔNG phải Chiến sĩ
            if (m.position && m.position !== 'Chiến sĩ') return true;
            // Loại bỏ các trường hợp còn lại (member/Chiến sĩ)
            return false;
        });
        console.log('[Members] 🔒 Filtered BCH only:', originalCount, '→', membersDataCache.length);
...
// [COMMENTED OUT] Định nghĩa này đã bị ghi đè bởi hard delete ở dưới (dòng ~2539)
// Hard delete version có thêm logic xóa bản ghi trùng email
/*
window.deleteMember = async function (memberId) {
    // ... soft delete code commented out ...
};
*/
