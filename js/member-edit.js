...
window.deleteMember = async function (memberId) {
    const member = membersDataCache.find(m => m.id === memberId);
    if (!member) {
        console.warn('[Delete Member] Not found in cache:', memberId);
        await showAlert('Không tìm thấy thành viên trong danh sách cache', 'error');
        return;
    }
    ...
    try {
        console.log('[Delete Member] Deleting:', memberId, member.email);
        ...
        // Nếu doc không tồn tại trong Firebase, vẫn xóa khỏi view
        if (error.code === 'not-found' || error.message?.includes('not-found')) {
            localStorage.removeItem('xtn_members_cache');
            await showAlert('Đã xóa khỏi danh sách!', 'success');
            loadMembers();
        } else {
